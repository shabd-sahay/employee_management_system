...
from shabd.models import Company,Employee
from shabd.forms import CompanyForm,EmployeeForm
...
def employee_list(request):
    # Get the unique company names for filtering
    company_names = Employee.objects.values_list('eCompany', flat=True).distinct()

    # Retrieve the employees
    employees = Employee.objects.all()

    # Apply filtering by company name
    company_filter = request.GET.get('company_filter')
    if company_filter:
        employees = employees.filter(eCompany=company_filter)

    # Apply search by first name
    search_query = request.GET.get('search_query')
    if search_query:
        employees = employees.filter(eFname__icontains=search_query)
